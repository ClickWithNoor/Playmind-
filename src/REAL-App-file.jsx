...
import { MessageCircle, Brain, Wrench, Gamepad2, Crown, Send, RefreshCw, Sparkles, Lock, Check, X, Gift, Copy, Trophy, Info, Mail, ShieldCheck, BookOpen } from "lucide-react";
...
function InviteTab({ account, setAccount, refreshAccount, creditReferralIfNeeded, setTab }) {
  ...
  async function createAccount() {
    ...
    try {
      ...
      const result = await storage.set(`ref:${newCode}`, JSON.stringify(data));
      if (!result) throw new Error("Storage did not confirm the save.");
      setAccount({ code: newCode, name: data.name, points: 0, premium: false });
      creditReferralIfNeeded?.(newCode);
    } catch (e) {
      setError("Could not create your account — " + (e?.message || "please try again."));
    }
    setCreating(false);
  }
      ...
      <div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 mb-4">
          ...
          <button
            ...
            disabled={creating}
            className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white text-sm font-medium py-2.5 rounded-lg"
          >
            {creating ? "Creating…" : "Get my invite link"}
          </button>
        ...
        {!account.premium ? (
          ...
          <>
            <p className="text-xs text-slate-500 mb-3">
              Rs 100 needs {PAYOUT_TIERS[0].points} pts · Rs 300 needs {PAYOUT_TIERS[1].points} pts. You have {points}.
            </p>
            {redeemSent ? (
...
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// Blog / Articles tab — a few short, original written pieces.
// Real written content like this (not just interactive tools) is
// one of the things AdSense reviewers look for.
// ---------------------------------------------------------------
const ARTICLES = [
  {
    id: "trivia-tips",
    title: "5 Tricks to Get Better at Trivia Quizzes",
    excerpt: "Small habits that make a real difference over time.",
    body: [
      "Trivia rewards curiosity more than memorization. If you enjoy the quiz on this site, here are a few habits that tend to help over time.",
      "First, read widely instead of narrowly. People who do well at general knowledge quizzes usually consume a mix of news, documentaries, and even random Wikipedia rabbit holes, rather than studying one topic hard.",
      "Second, pay attention to categories you're weak in. It's tempting to keep playing the categories you're already good at, but real improvement comes from spending a little extra time on your weaker ones.",
      "Third, learn from wrong answers. When you miss a question, take five seconds to actually register the correct answer instead of just moving to the next one — repetition is what makes facts stick.",
      "Fourth, play often but in short bursts. Five minutes a day beats one long session a week for building recall.",
      "Finally, don't overthink simple questions. A lot of quiz mistakes come from second-guessing an answer that was correct on first instinct.",
    ],
  },
  {
    id: "invite-tips",
    title: "How to Get the Most Out of the Invite Program",
    excerpt: "A few honest tips for sharing your link effectively.",
    body: [
      "The invite program on this site rewards you when someone you've shared your link with actually creates or logs into an account — not just for clicking the link. That means the way you share it matters.",
      "Share with people who'll genuinely find the site useful — a quick quiz, a game, and a few handy tools — rather than mass-spamming strangers. Genuine shares convert into real accounts far more often.",
      "A short, honest message works better than an exaggerated one. Something like 'I've been using this site for quick trivia breaks, check it out' tends to get more real signups than an over-the-top pitch.",
      "Group chats and status updates on WhatsApp tend to outperform one-off DMs, simply because more people see them.",
      "Be patient — points add up gradually, and payouts are reviewed manually, so allow a little time after you hit a payout tier.",
    ],
  },
  {
    id: "2048-strategy",
    title: "A Simple Strategy for 2048",
    excerpt: "One habit that consistently improves your score.",
    body: [
      "2048 looks simple but rewards a bit of strategy. The single biggest improvement most players can make is picking one corner and sticking to it.",
      "Choose a corner — say, bottom-left — and try to always keep your highest-value tile pinned there. Avoid moves that would shift it away from that corner unless absolutely necessary.",
      "Build your board so that tiles increase in value along one edge, like a staircase leading into your chosen corner. This keeps merges predictable instead of chaotic.",
      "Try to avoid using the direction that moves away from your corner unless you have no other legal move. Over time this single habit alone tends to push scores noticeably higher.",
      "Finally, don't panic when the board fills up — a full board isn't game over as long as a legal merge is still available somewhere.",
    ],
  },
  {
    id: "small-money-habits",
    title: "Small Habits for Managing Small Amounts of Money",
    excerpt: "Practical thoughts for anyone earning a bit here and there.",
    body: [
      "Earning small amounts through things like referral programs can add up, but only if it's paired with a habit of actually keeping track of it.",
      "A simple approach: the moment a small payout lands, move it somewhere separate from your daily spending money — even a separate mobile wallet balance counts. Small amounts left mixed in with everyday spending tend to just disappear unnoticed.",
      "Set a small, concrete goal for what the money is for, even if it's minor — a specific snack, topping up mobile credit, or saving toward something a bit bigger. Goals make small amounts feel worth tracking.",
      "Avoid chasing programs that promise unusually high returns for very small amounts of money — if a payout seems disproportionate to the effort involved, it's worth being cautious rather than assuming it's simply generous.",
    ],
  },
];
  ...
  if (article) {
    return (
      <div className="max-w-2xl">
        <button onClick={() => setOpenId(null)} className="text-xs text-slate-400 mb-3">← Back to articles</button>
      ...
      <div className="space-y-3">
        {ARTICLES.map((a) => (
          <button
            ...
            className="w-full text-left bg-slate-900 border border-slate-800 hover:border-indigo-500 rounded-xl p-4"
          >
            <p className="font-semibold text-sm mb-1">{a.title}</p>
            <p className="text-xs text-slate-400">{a.excerpt}</p>
          </button>
        ))}
      </div>
    </div>
